...
from django.test import override_settings  # Import settings override decorator.
from django.urls import reverse  # Import URL resolver.
from rest_framework.test import APITestCase  # Import DRF API test case.
from rest_framework import status  # Import HTTP status codes.

from Blog import models as Blog_models  # Import blog models.

# Get the project's custom user model.
User = get_user_model()

# Override the email backend during tests.
EMAIL_OVERRIDE = override_settings(
    EMAIL_BACKEND="django.core.mail.backends.locale.EmailBackend"
)


# Test authentication and permission-related API endpoints.
@EMAIL_OVERRIDE
